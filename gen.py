...
if __name__ == '__main__':
    ...
    #################################
    t2m_transformer = load_trans_model(model_opt, opt, 'net_best_acc.tar')  # 'latest.tar'
    ...
    print(f'Video encoding time: {(encoder_end_time - encoder_start_time)*1000:.2f} ms')
    ...
    for r in range(opt.repeat_times):
        ...
        with torch.no_grad():
            ...
            print(f'ID generation time: {(id_end_time - id_start_time)*1000:.2f} ms')
            
            decoder_start_time = time.time()
            pred_motions = vq_model.forward_decoder(mids)
            decoder_end_time = time.time()
            print(f'Decoding time: {(decoder_end_time - decoder_start_time)*1000:.2f} ms')
