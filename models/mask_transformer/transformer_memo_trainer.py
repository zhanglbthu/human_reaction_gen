...
from utils.eval_vimo import evaluation_mask_transformer_memo, evaluation_res_transformer_memo
...
class MaskTransformerTrainer:
    def __init__(self, args, t2m_transformer, vq_model, video_encoder):
        self.opt = args
        self.t2m_transformer = t2m_transformer  # note not specific to t2m, too lazy to rename
    ...
    def forward(self, batch_data):
        ...
        imgs, motion, m_lens, video_path, cam_traj = batch_data # imgs: [B, T, C, H, W]
        '''
        ...
        _loss, _pred_ids, _acc = self.t2m_transformer(code_idx[..., 0], conds, m_lens, at_features, cam_conds=cam_traj)
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        if self.opt.finetune:
            model_dir = pjoin(self.opt.finetune_model_dir, 'latest.tar')
            checkpoint = torch.load(model_dir, map_location=self.device)
            print("Loading model from: %s"%(model_dir))
            model_key = 't2m_transformer' if 't2m_transformer' in checkpoint else 'net' # too lazy to rename
        ...
        while epoch < self.opt.max_epoch:
            ...
            if np.mean(val_loss) < best_loss:
                print(f"Improved loss from {best_loss} to {np.mean(val_loss)}!!!")
                # self.save(pjoin(self.opt.model_dir, 'net_best_loss.tar'), epoch, it)
                best_loss = np.mean(val_loss)

            if np.mean(val_acc) > best_acc:
                print(f"Improved accuracy from {best_acc} to {np.mean(val_acc)}!!!")
...
class ResidualTransformerTrainer:
    def __init__(self, args, res_transformer, vq_model, video_encoder):
        self.opt = args
    ...
    def forward(self, batch_data):
        ...
        ce_loss, pred_ids, acc = self.res_transformer(code_idx, conds, m_lens, at_features)
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        if self.opt.finetune:
            ...
            checkpoint = torch.load(model_dir, map_location=self.device)
            print("Loading model from: %s"%(model_dir))
            model_key = 'res_transformer' if 'res_transformer' in checkpoint else 'net'
            self.res_transformer.load_state_dict(checkpoint[model_key])

        self.opt_res_transformer = optim.AdamW(self.res_transformer.parameters(), betas=(0.9, 0.99), lr=self.opt.lr, weight_decay=1e-5)
        #self.opt_res_transformer = optim.AdamW(list(self.fuser.parameters()), betas=(0.9, 0.99), lr=self.opt.lr, weight_decay=1e-5) # frozen trans
        ...
        while epoch < self.opt.max_epoch:
            ...
            if np.mean(val_acc) > best_acc:
                print(f"Improved acc from {best_acc} to {np.mean(val_acc)}!!!")
                # self.save(pjoin(self.opt.model_dir, 'net_best_acc.tar'), epoch, it)
                best_acc = np.mean(val_acc)
