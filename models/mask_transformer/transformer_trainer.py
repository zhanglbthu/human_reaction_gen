...
from utils.eval_vimo import evaluation_mask_transformer, evaluation_res_transformer
...
class MaskTransformerTrainer:
    ...
    def forward(self, batch_data):
        ...
        conds, motion, m_lens, video_path = batch_data
        conds, _ = self.video_encoder(conds.cuda())
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        best_fid, best_div, writer = evaluation_mask_transformer(
            self.opt.save_root, eval_val_loader, self.t2m_transformer, self.vq_model, self.video_encoder, self.logger, epoch,
...
class ResidualTransformerTrainer:
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        best_fid, best_div, writer = evaluation_res_transformer(
            self.opt.save_root, eval_val_loader, self.res_transformer, self.vq_model, self.video_encoder, self.logger, epoch,
            best_fid=100, best_div=100,
            eval_wrapper=eval_wrapper, plot_func=plot_eval, 
            save_ckpt=True, save_anim=True
        )
